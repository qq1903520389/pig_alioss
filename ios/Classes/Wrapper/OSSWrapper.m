...
- (void)asyncGetImage:(NSString *)objectKey andProgress:(void (^)(float progress))uploadProgress success:(void (^)(id _Nullable))success failure:(void (^)(NSError * _Nonnull))failure {
    ...
    _normalDloadRequest.bucketName = pigAliOssService.bucketName;
...
- (void)asyncPutImage:(NSString *)objectKey localFilePath:(NSString *)filePath andProgress:(void (^)(float progress))uploadProgress success:(void (^)(id _Nullable))success failure:(void (^)(NSError * _Nonnull))failure {
    if (![objectKey oss_isNotEmpty]) {
        NSError *error = [NSError errorWithDomain:NSInvalidArgumentException code:0 userInfo:@{NSLocalizedDescriptionKey: @"objectKey should not be nil"}];
        failure(error);
        return;
    ...
    _normalUploadRequest.bucketName = pigAliOssService.bucketName;
...
//        OSSLogDebug(@"上传文件进度: %f", progress);
    ...
    _normalUploadRequest.callbackParam = @{
                                           @"callbackUrl": pigAliOssService.callback,
                                           // callbackBody可自定义传入的信息
                                           @"callbackBody":  [NSString stringWithFormat:@"filename=%@",objectKey]
...
- (void)textWaterMark:(NSString *)object waterText:(NSString *)text objectSize:(int)size success:(void (^)(id _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure {
    ...
    [self asyncGetImage:[NSString stringWithFormat:@"%@%@", object, queryString] andProgress:^(float progress) {
        OSSLogDebug(@"下载文件进度: %f", progress)
