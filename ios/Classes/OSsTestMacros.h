//
//  OSSTestMacros.h
//  AliyunOSSiOSTests
//
//  Created by 怀叙 on 2017/12/11.
//  Copyright © 2017年 阿里云. All rights reserved.
//

#ifndef OSSTestMacros_h
#define OSSTestMacros_h

#define OSS_ACCESSKEY_ID                @"LTAI68qVwYphByP"                              // 子账号id
#define OSS_SECRETKEY_ID                @"hTQfDtPg4qt7N48"                          // 子账号secret

#define OSS_BUCKET_PUBLIC               @"public-bucket"                            // bucket名称
#define OSS_BUCKET_PRIVATE              @"pigeon-shop-app"                           // bucket名称private-bucket
#define OSS_ENDPOINT                    @"https://oss-cn-beijing.aliyuncs.com/"      // 访问的阿里云endpoint http://oss-cn-region.aliyuncs.com
#define OSS_IMG_ENDPOINT                @"https://oss-cn-beijing.aliyuncs.com/"      // 旧版本图片服务的endpoint
#define OSS_MULTIPART_UPLOADKEY         @"multipart_key"                            // 分片上传的object key
#define OSS_RESUMABLE_UPLOADKEY         @"resumable_key"                            // 断点续传的object key
#define OSS_CALLBACK_URL                @"http://43.143.203.228:"       // 对象上传成功时回调的业务服务器地址http://oss-demo.aliyuncs.com:23450
#define OSS_CNAME_URL                   @"http://www.cnametest.com/"                // cname，用于替换bucket.endpoint的访问域名
#define OSS_STSTOKEN_URL                @"http://43.143.20"           // sts授权服务器的地址
#define OSS_IMAGE_KEY                   @"testImage.png"                            // 测试图片的名称

#define OSS_DOWNLOAD_FILE_NAME          @"OSS_DOWNLOAD_FILE_NAME"                   // 用于下载的object key

#endif /* OSSTestMacros_h */
